...
const PortraitPhoto = ({ className }: { className?: string }) => {
  ...
  return (
    <div className={`relative group ${className}`}>
      ...
      <div className="relative border-4 border-[var(--bg-card)] shadow-xl overflow-hidden bg-slate-50 dark:bg-slate-800 rounded-sm inline-block">
        {!hasError ? (
          <img 
            src={imageUrl} 
            alt="স্বর্গীয় বিভু রঞ্জন সরকার"
            referrerPolicy="no-referrer"
            className="max-w-full h-auto grayscale brightness-[1.05] dark:brightness-[0.8] hover:grayscale-0 transition-all duration-700 block"
            onError={() => setHasError(true)}
          />
        ) : (
          <div className="flex flex-col items-center justify-center text-slate-300 dark:text-slate-600 p-8 text-center space-y-4">
            <div className="w-16 h-16 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center">
              <User className="w-8 h-8 opacity-40" />
            </div>
            <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500">চিত্র পাওয়া যায়নি</p>
          </div>
        )}
      </div>
    </div>
...
const FeatureItem = ({ icon, title, desc }: { icon: React.ReactNode, title: string, desc: string }) => (
  <div className="flex gap-4 items-start group">
    <div className="w-10 h-10 rounded-2xl bg-emerald-50 dark:bg-emerald-900/40 text-emerald-600 dark:text-emerald-400 flex items-center justify-center shrink-0 group-hover:bg-emerald-500 group-hover:text-white transition-all duration-300">
      {icon}
    </div>
...
const InfoView: React.FC = () => {
  return (
    <div className="max-w-4xl mx-auto p-4 pt-6 space-y-6 animate-in fade-in duration-1000 font-bengali pb-32">
      {/* Dedication Section */}
      <div className="bg-[var(--bg-card)] p-6 md:p-8 rounded-3xl shadow-xl shadow-slate-200/50 dark:shadow-black/20 border border-[var(--border-color)] flex flex-col items-center text-center space-y-8">
      ...
      <div className="bg-[var(--bg-card)] p-6 md:p-8 rounded-3xl shadow-xl shadow-slate-200/50 dark:shadow-black/20 border border-[var(--border-color)] flex flex-col items-center text-center space-y-6">
        ...
        <div className="space-y-5 text-slate-700 dark:text-slate-300 leading-relaxed font-medium max-w-2xl">
          <p className="text-emerald-600 dark:text-emerald-500 font-bold italic text-lg border-l-4 border-emerald-500 pl-4 py-1 bg-emerald-50/30 dark:bg-emerald-900/10 rounded-r-xl">
      ...
      <div className="bg-[var(--bg-card)] p-6 md:p-8 rounded-3xl shadow-xl shadow-slate-200/50 dark:shadow-black/20 border border-[var(--border-color)] flex flex-col items-center text-center space-y-8">
        ...
        <div className="grid gap-8 sm:grid-cols-2 text-left w-full">
          ...
          <FeatureItem 
            ...
            title="সহজ পঠনযোগ্যতা ও ডার্ক মোড"
            desc="পড়ার সুবিধার্থে ফন্টের আকার পরিবর্তন, বিভিন্ন বাংলা ফন্ট এবং চোখের আরামের জন্য ডার্ক মোড (Dark Mode) সুবিধা।"
          />
          <FeatureItem 
      ...
      <div className="bg-[var(--bg-card)] p-6 md:p-8 rounded-3xl shadow-xl shadow-slate-200/50 dark:shadow-black/20 border border-[var(--border-color)] flex flex-col items-center text-center space-y-6">
        ...
        <div className="space-y-4 text-slate-700 dark:text-slate-300 leading-relaxed font-medium max-w-2xl">
          <p className="text-[15px]">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
            <div className="p-4 bg-emerald-50/50 dark:bg-emerald-900/10 rounded-2xl border border-emerald-100/50 dark:border-emerald-800/30 text-left">
              <h5 className="font-bold text-emerald-700 dark:text-emerald-400 mb-1">নির্ভুলতা</h5>
              <p className="text-xs opacity-80">মূল গীত পুস্তকের সাথে সামঞ্জস্য রেখে প্রতিটি গানের বানান ও যতিচিহ্ন যাচাই করা হয়েছে।</p>
            </div>
            <div className="p-4 bg-emerald-50/50 dark:bg-emerald-900/10 rounded-2xl border border-emerald-100/50 dark:border-emerald-800/30 text-left">
              <h5 className="font-bold text-emerald-700 dark:text-emerald-400 mb-1">গতি ও সাবলীলতা</h5>
      ...
      <div className="bg-emerald-600 p-6 md:p-8 rounded-3xl shadow-xl shadow-emerald-200/40 text-white flex flex-col items-center text-center space-y-5">
        ...
        <div className="pt-2">
          <a
            ...
            className="inline-flex items-center gap-2 px-6 py-3 bg-white dark:bg-slate-100 text-emerald-600 dark:text-emerald-700 rounded-2xl font-bold text-sm hover:bg-emerald-50 dark:hover:bg-white transition-colors shadow-lg shadow-black/5"
          >
      ...
      <div className="text-center pt-8 space-y-4">
        <div className="flex items-center justify-center gap-2 text-slate-200 dark:text-slate-700">
